            ...
            for direction in directions:
                ...
                while directions[direction] and magnitude <= self.rules.retrieve_max_bomb_magnitude(possible_players):
                    ...
                    if injured_element is not None:
                        ...
                        if injured_element.is_bomb() and bomb.type == Element.TIMER_1_LEFT_BOMB:
                        ...
                        if injured_element.bomb_threat is None \
                                or injured_element.bomb_threat.type == Element.TIMER_1_LEFT_BOMB:
    ...
    def move_or_put_a_bomb(self, next_place, path):
        ...
        if not next_place.is_bomb_range():
            ...
            if self.board.bomber_killer.individual_effort <= 40 and self.can_put_bomb():
                ...
                # If an enemy is 4 spaces, randomize a 50% for put a bomb
                if is_safe_put_a_bomb and len(path) >= 4 and Random().random() < 0.5:
                    if path[3].is_player():
                        action = self.put_a_bomb_on(next_place)

                # If an enemy is 3 spaces, randomize a 75% for put a bomb
